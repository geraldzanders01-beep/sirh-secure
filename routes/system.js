...
router.all("/read-report", async (req, res) => {
    ...
    try {
        ...
        if (period === 'today') {
            const latestByEmp = {};
            (pointages || []).forEach(p => { latestByEmp[p.employee_id] = p; });
        ...
        // --- MODE MENSUEL ---
            ...
            for (const empId in pointsByEmp) {
                ...
                if (!monthlyStats[empId]) monthlyStats[empId] = { nom: empInfo.nom, totalMs: 0, joursPresence: new Set() };

                let pendingInTime = null;
                events.forEach(ev => {
                    const evTime = new Date(ev.heure).getTime();
                    if (ev.action === 'CLOCK_IN') {
                        if (pendingInTime) monthlyStats[empId].totalMs += (calculateAutoClose(pendingInTime, isSecurity) - pendingInTime);
                        pendingInTime = evTime;
                        monthlyStats[empId].joursPresence.add(new Date(ev.heure).toLocaleDateString());
                    } else if (ev.action === 'CLOCK_OUT' && pendingInTime) {
                ...
                if (pendingInTime) {
                    if (new Date(pendingInTime).toLocaleDateString() === now.toLocaleDateString()) {
            ...
            const finalReport = Object.values(monthlyStats).map(s => {
                ...
                return {
                    ...
                    heures: `${Math.floor(totalMins / 60)}h ${(totalMins % 60).toString().padStart(2, '0')}m`,
                    statut: "Validé"
                };
            });
